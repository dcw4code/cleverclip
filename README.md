...
```
source_clips/ 全部 .mp4 → 逐个固定频率抽帧 → LLM 视觉识别帧内容 → 统一时间轴
                                                                                                                                            ↓
最终视频 ← ffmpeg 裁切+拼装 ← LLM 规划剪辑片段（跨视频混剪） ← 自然语言剪辑需求
```
...
- `1.0`：每秒一帧（默认，适合短视频）
- `10.0`：每两秒一帧（适合长视频，节省 API 调用）
- `0.5`：每半秒一帧（高精度，但 API 成本高）

## 注意事项

1. **API 费用**：视觉识别会发送图片到 LLM，注意 API 调用成本
2. **临时空间**：抽帧会生成大量图片，确保磁盘空间充足
3. **视频编码**：输出使用 H.264 + AAC，兼容性较好
4. **多视频混剪**：不同来源视频的片段会被按规划顺序拼接，最终输出为单一视频
