...
# 剪辑规划系统提示词
PLANNER_SYSTEM_PROMPT = """你是一个专业的视频剪辑规划助手。用户会给你多段视频的时间轴分析结果（包含来源视频、时间点和内容描述），以及一段剪辑需求。

请根据需求和内容分析，规划出需要剪辑的视频片段，返回 JSON 格式：

{
  "clips": [
    {
      "source": "来源视频文件名（不含扩展名，与时间轴中的 source 字段一致）",
      "start": 开始时间（秒，数字）,
      "end": 结束时间（秒，数字）,
      "reason": "选择这段的原因（中文）"
    }
...
class LLMClient:
    ...
    def plan_clips(self, timeline: list[dict], requirement: str) -> list[dict]:
        """
        根据时间轴和用户需求，生成剪辑片段方案。
        ...
        console.print(f"[green]✓ 规划了 {len(clips)} 个剪辑片段[/green]")
        return clips
